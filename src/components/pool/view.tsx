...
const PoolItem = (props: { item: { pool: PoolInfo, isFeeAccount: boolean, account: TokenAccount } }) => {
  ...
  if (item) {
    return <div className="pool-item-row" onClick={setPair} title={`LP Token: ${props.item.pool.pubkeys.mint.toBase58()}`}>
      <div className="pool-item-amount">{amount.toFixed(4)}</div>
      <div className="pool-item-type" title="Fee account">{item.isFeeAccount ? ' (F) ' : ' '}</div>
