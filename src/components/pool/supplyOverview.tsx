...
const useMidPriceInUSD = (mint: string) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      let market = await Market.load(connection, marketInfo.address, undefined, marketInfo.programId);
  ...
  return (<Card style={{ borderWidth: 0 }}>
    <div style={{ display: 'flex' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', marginLeft: 20, flex: '1 1', alignItems: 'flex-start', justifyContent: 'center' }}>
        <div>
          <span>{data[0].name}:</span> {formatTokenAmount(accountA, mintA)} {!isBaseA && formatTokenAmount(accountA, mintA, priceA, '($', ')')}
        </div>
        <div>
          <span>{data[1].name}:</span> {formatTokenAmount(accountB, mintB)}  {!isBaseB && priceB && formatTokenAmount(accountB, mintB, priceB, '($', ')')}
        </div>
      </div>
    </div>
  </Card>);
